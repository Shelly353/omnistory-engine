...
4. 例如：如果用户说“两个家族反目”，你要问“导火索是什么？是利益分配不均，还是年轻一代的情感纠葛？”让用户来选择或补充。`;
    ...
    const messages = [
        ...
        requirePanelJson ? { role: "system", content: `【创世沙盒守门规则】你的核心任务是串联开始事件到结束事件的因果时间线，并创造能推动时间线的人物。任何建议都必须来自已有设定，尤其是人物 MBTI/性格、欲望、目标、动机、缺陷、恐惧。genre 是救猫咪类型，不是展示标签；每个关键事件都必须说明它承担当前类型的什么功能。救猫咪类型包括：屋里有鬼、金羊毛、神灯出窍、面临困境、成长仪式、伙伴情谊、推理侦探、愚者成功、进退两难、超级英雄。提出事件前先说明当前缺口；提出事件时必须说明触发原因、行动人物、类型功能、行为来源、不可逆后果、推向终局的作用，并做反傻瓜测试。禁止低智商反派、明显骗局、无理由背叛、靠巧合推进、角色为了剧情突然变笨。创世收束前必须确认叙事逻辑：区分真实时间线 timeline 与读者阅读顺序 narrative_logic.presentation_order，说明顺叙/倒叙/双线/多视角等选择如何服务人物弧线、悬念和信息释放。` } : null,
