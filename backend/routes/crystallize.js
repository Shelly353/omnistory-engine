    ...
    try {
        ...
        const dsResponse = await fetch('https://api.deepseek.com/chat/completions', {
            ...
            body: JSON.stringify({
                ...
                messages: [{ role: "system", content: "你是一个严格的JSON提取器。请提取对话中的设定，必须包含 genre、worldview、rules、characters、relations、timeline、narrative_logic、chapters；narrative_logic 需要包含 mode、description、presentation_order。事件、人物、规则/专家三个模块互相影响；规则/世界观/专家资料权限最高。不符合规则、专业流程或人物逻辑的事件，要把警报和整改约束写入 rules。人物应尽量绑定到具体 timeline/chapters；参与事件少于三个的人物，要在人物简介或弧光中保留后续复用提示，避免一次性人物。律师、医生、警察、金融、政治、文化、种族、技能等专业关键词对应的专家资料也合并进 rules。" }, { role: "user", content: userContent }],
                temperature: 0.1 
            })
        });
