...
5. 不要写正文。`;
    ...
    function getSopOutlineGateReport() {
        const trail = currentChapterChatHistory.map(msg => msg.content || '').join('\n');
        const checks = [
...
如果本次动作是半自动或手动，请不要输出 JSON。`;
    ...
    function findLeadCharacter(bible = {}, type = 'protagonist') {
        ...
        return chars.find(c => pattern.test(`${c.role || ''} ${c.description || ''} ${c.name || ''}`));
    }
    ...
    function hasBasicCharacterDecisionCard(character = {}) {
        ...
        const limit = character.character_rules || character.skills || character.description;
        return !!(
...
如果没有问题，请明确“当前可继续”。`;
        ...
        try {
            ...
            if (/红色警报[\s\S]*(无法|矛盾|泄露|降智|阻断)|不通过|严重/.test(reply)) {
                setSopSupervisionBlocked(true, reply);
