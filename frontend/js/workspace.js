...
1.1 下一事件只作为“不可越界的过渡边界”，不得引用其具体细节、不得追问下一事件、不得给出下一事件方案。
2. 不要重复追问已回答问题。
...
6. 不要写正文。`;
    ...
    function buildInformationFirewallPrompt() {
        return `【信息权限防火墙：SOP/正文必须继承沙盒设定但区分视角】
1. 沙盒 secrets 中 status=hidden/partial 的 god_view 是作者与 AI 的后台因果校验资料，不能作为角色、观众或可见叙述已经知道的事实。
2. SOP 对话、SOP 提问、正文可见内容只能基于 audience_view 推进；不得向作者提出“要不要让角色知道上帝视角真相”这类泄密问题。
3. 如果需要使用隐藏真相，只能转化为：观众视角可见线索、误导、伏笔、信息差、角色误判、调查方向；不要直接说出 god_view 的真相细节。
4. status=revealed 后，god_view 才能作为公开事实进入 SOP 和正文。
5. 如果 audience_view 缺失而 god_view 存在，必须触发警告：先补观众视角，再继续 SOP/正文。`;
    }

    function buildSopCanonPrompt() {
        const currentBible = getCurrentBibleSnapshot() || {};
        const eventContext = getAdjacentEventContext(currentLocalContext.chapterNumber);
        const bibleChapter = findBibleChapterForContext(currentLocalContext.chapterNumber, currentLocalContext.title, currentBible);
        ...
        const bibleSnapshot = limitText(JSON.stringify(compactBibleForPrompt(currentBible) || {}, null, 2), 7000);
        return `【沙盒最终设定继承契约：SOP/正文最高优先级】
1. SOP、事件梗概、大纲、正文、审查和修复必须完全继承沙盒 Bible 的每一项已确认设定。
2. 如果旧聊天记录、数据库旧大纲、旧人物称谓或 AI 曾经生成的旧方案与沙盒 Bible 冲突，以沙盒 Bible 和右侧实时面板最新内容为准。
3. 不得凭空新增或修改人物身份、职位、关系、能力、死因、事件顺序、隐藏真相、规则、器物、货币、称谓或时代语感。
4. 当前操作只处理当前事件；下一事件只作为结尾边界，不得展开下一事件的具体细节。
...
【沙盒中匹配的当前事件】
${bibleChapter ? JSON.stringify(compactBibleForPrompt(bibleChapter), null, 2) : '未在 Bible chapters 中找到完全匹配项，请以当前事件边界和最新面板为准。'}
...
${bibleSnapshot}`;
    ...
    function renderSopSupervisionGate(reason = '') {
        ...
        `;
        document.getElementById('btn-sop-gate-discuss')?.addEventListener('click', () => openSopWarningFixModal(reason));
        ...
        setTimeout(() => openSopWarningFixModal(reason), 0);
        if (window.lucide) lucide.createIcons();
    }

    function ensureSopWarningFixModal() {
        let modal = document.getElementById('sop-warning-fix-modal');
        if (modal) return modal;
        document.body.insertAdjacentHTML('beforeend', `
            <div id="sop-warning-fix-modal" class="fixed inset-0 bg-black/90 backdrop-blur-md z-[88] hidden flex items-center justify-center p-6">
                <div class="bg-gray-950 border border-red-700/70 rounded-2xl shadow-2xl w-full max-w-4xl max-h-[88vh] flex flex-col overflow-hidden">
                    <div class="p-5 border-b border-red-900/60 bg-red-950/20 flex items-start justify-between gap-4">
                        <div>
                            <div class="text-[11px] font-bold tracking-wider text-red-300 mb-1">SOP 阻断警告 · 独立修复</div>
                            <h3 class="text-xl font-bold text-white">先消除警告，再继续提问</h3>
                            <div class="text-xs text-gray-400 mt-1">这里的讨论不会混入主 SOP 对话。修复后请重新检查，红色警告消失前不能继续普通推演。</div>
                        </div>
                        <button id="btn-close-sop-warning-fix" class="text-gray-500 hover:text-white p-1 rounded border border-transparent hover:border-gray-700"><i data-lucide="x" class="w-5 h-5"></i></button>
                    </div>
                    <div class="p-5 overflow-y-auto space-y-4">
                        <div class="text-xs text-red-100 bg-red-950/30 border border-red-800/60 rounded-xl p-3 whitespace-pre-wrap" id="sop-warning-fix-reason"></div>
                        <div id="sop-warning-fix-chat" class="space-y-3 text-xs"></div>
                    </div>
                    <div class="p-4 border-t border-gray-800 bg-gray-950">
                        <textarea id="sop-warning-fix-input" class="w-full h-24 bg-gray-900 border border-gray-700 rounded-xl p-3 text-sm text-white resize-none" placeholder="只讨论如何修复这条警告，例如：按建议改当前事件的信息释放方式..."></textarea>
                        <div class="grid grid-cols-3 gap-2 mt-3">
                            <button id="btn-send-sop-warning-fix" class="py-2 bg-red-700 hover:bg-red-600 text-white rounded-lg font-bold text-sm">讨论修复</button>
                            <button id="btn-recheck-sop-warning-fix" class="py-2 bg-amber-700 hover:bg-amber-600 text-white rounded-lg font-bold text-sm">重新检查</button>
                            <button id="btn-ignore-sop-warning-fix" class="py-2 bg-gray-800 hover:bg-gray-700 text-gray-100 rounded-lg font-bold text-sm">暂时忽略</button>
                        </div>
                    </div>
                </div>
            </div>
        `);
        modal = document.getElementById('sop-warning-fix-modal');
        document.getElementById('btn-close-sop-warning-fix')?.addEventListener('click', () => modal.classList.add('hidden'));
        document.getElementById('btn-send-sop-warning-fix')?.addEventListener('click', sendSopWarningFixMessage);
        document.getElementById('btn-recheck-sop-warning-fix')?.addEventListener('click', () => runSopRealtimeSupervision('警告修复后重新检查'));
        document.getElementById('btn-ignore-sop-warning-fix')?.addEventListener('click', () => {
            setSopSupervisionBlocked(false);
            modal.classList.add('hidden');
            renderDeviationItems([`已暂时忽略 SOP 红色警告。\n\n${limitText(sopWarningFixState.reason, 900)}`]);
        });
        if (window.lucide) lucide.createIcons();
        return modal;
    }

    function renderSopWarningFixModal() {
        const reasonBox = document.getElementById('sop-warning-fix-reason');
        const chatBox = document.getElementById('sop-warning-fix-chat');
        if (reasonBox) reasonBox.textContent = sopWarningFixState.reason || sopSupervisionGate.reason || '';
        if (chatBox) {
            chatBox.innerHTML = (sopWarningFixState.chat || []).map(msg => `
                <div class="${msg.role === 'user' ? 'ml-14 bg-red-900/40' : 'mr-14 bg-gray-800'} p-3 rounded-xl whitespace-pre-wrap leading-relaxed">${escapeHtml(msg.content || '')}</div>
            `).join('');
            chatBox.scrollTop = chatBox.scrollHeight;
        }
    }

    function openSopWarningFixModal(reason = sopSupervisionGate.reason) {
        sopWarningFixState = {
            reason: reason || sopSupervisionGate.reason || '',
            chat: [{ role: 'assistant', content: `我会先处理这条阻断警告。请说明你想怎么改，或直接让我给最小修复方案。\n\n${limitText(reason || sopSupervisionGate.reason || '', 900)}` }]
        };
        const modal = ensureSopWarningFixModal();
        modal.classList.remove('hidden');
        renderSopWarningFixModal();
    }

    async function sendSopWarningFixMessage() {
        const input = document.getElementById('sop-warning-fix-input');
        const text = input?.value.trim();
        if (!text) return;
        input.value = '';
        sopWarningFixState.chat.push({ role: 'user', content: text });
        sopWarningFixState.chat.push({ role: 'assistant', content: '正在生成修复建议...' });
        renderSopWarningFixModal();
        const prompt = `你是 SOP 红色警告独立修复器。只处理警告，不推进新剧情，不向作者提出下一轮 SOP 问题。
...
【当前红色警告】
${sopWarningFixState.reason || sopSupervisionGate.reason}
...
不要输出新的普通 SOP 问题。`;
        try {
            ...
            sopWarningFixState.chat.pop();
            sopWarningFixState.chat.push({ role: 'assistant', content: data.success ? (stripFencedBlocks(data.reply) || data.reply) : `修复建议失败：${data.error || '未知错误'}` });
            renderSopWarningFixModal();
        } catch (error) {
            sopWarningFixState.chat.pop();
            sopWarningFixState.chat.push({ role: 'assistant', content: `修复建议失败：${error.message || '未知错误'}` });
            renderSopWarningFixModal();
        }
    }
    ...
    function getSopOutlineGateReport() {
        ...
        const checks = [
            ...
            { label: '信息释放/观众视角/揭露权限', ok: /信息释放|观众视角|信息差|揭露|伏笔权限/.test(trail) },
    ...
    function saveSopEventBriefState(state = sopEventBriefState, chapterId = state.chapterId || currentLocalContext.chapterId) {
        const targetChapterId = chapterId || currentLocalContext.chapterId;
        if (!targetChapterId) return;
        sopEventBriefState = { ...state, chapterId: targetChapterId };
        localStorage.setItem(getSopEventBriefKey(targetChapterId), JSON.stringify(sopEventBriefState));
    }

    function getSopEventBriefModalContext() {
        const modal = document.getElementById('sop-event-brief-modal');
        return {
            chapterId: modal?.dataset.chapterId || sopEventBriefState.chapterId || currentLocalContext.chapterId,
            chapterNumber: modal?.dataset.chapterNumber || currentLocalContext.chapterNumber,
            title: modal?.dataset.title || currentLocalContext.title
        };
    }

    function setSopEventBriefModalContext(context = currentLocalContext) {
        const modal = ensureSopEventBriefModal();
        modal.dataset.chapterId = context.chapterId || '';
        modal.dataset.chapterNumber = context.chapterNumber || '';
        modal.dataset.title = context.title || '';
        const label = document.getElementById('sop-event-brief-event-label');
        if (label) label.textContent = `当前编辑：事件 ${context.chapterNumber || '?'}《${context.title || '未命名'}》`;
        return modal;
    }

    function getSopHistoryForChapter(chapterId = currentLocalContext.chapterId) {
        if (!chapterId) return [];
        if (chapterId === currentLocalContext.chapterId) return currentChapterChatHistory;
        try {
            return JSON.parse(localStorage.getItem(`sop_v3_${PROJECT_ID}_${chapterId}`) || '[]');
        } catch (e) {
            return [];
        }
    }

    function saveSopHistoryForChapter(chapterId, history = []) {
        if (!chapterId) return;
        localStorage.setItem(`sop_v3_${PROJECT_ID}_${chapterId}`, JSON.stringify(history));
        if (chapterId === currentLocalContext.chapterId) currentChapterChatHistory = history;
    }
    ...
    function ensureSopEventBriefModal() {
                ...
                <div class="bg-gray-950 border border-emerald-700/70 rounded-2xl shadow-2xl w-full max-w-5xl max-h-[88vh] flex flex-col overflow-hidden">
                    <div class="p-5 border-b border-emerald-900/50 bg-emerald-950/20 flex items-start justify-between gap-4">
                        <div>
                            ...
                            <div id="sop-event-brief-event-label" class="text-xs text-emerald-300 mt-1"></div>
        ...
        document.getElementById('btn-save-sop-event-brief')?.addEventListener('click', () => {
            const ctx = getSopEventBriefModalContext();
            sopEventBriefState.brief = document.getElementById('sop-event-brief-text')?.value.trim() || '';
            sopEventBriefState.confirmed = false;
            saveSopEventBriefState({ ...sopEventBriefState, chapterId: ctx.chapterId }, ctx.chapterId);
            alert(`事件 ${ctx.chapterNumber || ''} 的故事梗概已保存。`);
        });
        document.getElementById('btn-confirm-sop-event-brief')?.addEventListener('click', () => {
            ...
            saveSopEventBriefState({ ...sopEventBriefState, chapterId: ctx.chapterId }, ctx.chapterId);
            const history = getSopHistoryForChapter(ctx.chapterId);
            history.push({ role: 'user', content: `【已确认的事件 ${ctx.chapterNumber || ''} 故事梗概】\n${brief}` });
            saveSopHistoryForChapter(ctx.chapterId, history);
            modal.classList.add('hidden');
            if (ctx.chapterId !== currentLocalContext.chapterId) {
                alert(`事件 ${ctx.chapterNumber || ''} 的故事梗概已确认。请切回该事件后再提取分章大纲。`);
                return;
            }
    ...
    function renderSopEventBriefModal() {
        ...
        const ctx = getSopEventBriefModalContext();
        const label = document.getElementById('sop-event-brief-event-label');
        if (label) label.textContent = `当前编辑：事件 ${ctx.chapterNumber || '?'}《${ctx.title || '未命名'}》`;
    ...
    async function generateSopEventBrief(context = getSopEventBriefModalContext()) {
        const history = getSopHistoryForChapter(context.chapterId);
        const prompt = `你是 SOP 当前事件总览编辑。请在进入分章大纲前，基于当前 SOP 讨论写一份“本事件故事梗概”，用于确认事件顺序和细节。

${buildSopCanonPrompt()}

注意：你现在只整理“事件 ${context.chapterNumber || ''}《${context.title || ''}》”。下一事件锚点只用于结尾过渡，不是本次修改对象；禁止输出下一事件的具体情节、人物行动、反派计划或设定细节。
【SOP 讨论记录】
${limitText(history.map(msg => `${msg.role}: ${stripSystemAppendix(msg.content || '')}`).join('\n\n'), 7000)}

要求：
1. 必须以【本事件故事梗概】开头。
2. 只讲当前事件，不展开下一事件。
3. 按事件发生顺序写清：起因、关键行动、阻力升级、人物选择、代价、观众视角信息释放、伏笔种植/回收、结尾如何停在后续边界前。
4. 单独输出【顺序与细节检查】，指出是否有不顺、跳步、巧合、动机不足或可修改的细节。
5. 不写正文，不写分章大纲。
6. 可见梗概不得泄露 hidden/partial 的上帝视角真相，只能写观众视角可见线索和信息差。`;
        const res = await fetch('/api/chat/deduce', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(buildChatPayloadWithLocalSources([{ role: 'user', content: prompt }], 1, prompt))
    ...
    async function sendSopEventBriefRevision() {
        const ctx = getSopEventBriefModalContext();
        if (sopEventBriefState.chapterId !== ctx.chapterId) loadSopEventBriefState(ctx.chapterId);
...
【当前修改对象】
事件 ${ctx.chapterNumber || ''}《${ctx.title || ''}》
注意：下一事件锚点只用于结尾过渡，不是本次反馈对象。你的反馈必须明确围绕事件 ${ctx.chapterNumber || ''}；禁止输出下一事件的具体情节、人物行动、反派计划或设定细节。
...
【SOP 讨论记录尾迹】
${limitText(history.slice(-8).map(msg => `${msg.role}: ${stripSystemAppendix(msg.content || '')}`).join('\n\n'), 5000)}

请输出更新后的完整【事件 ${ctx.chapterNumber || ''} 故事梗概】，并保留【顺序与细节检查】。不要把下一事件当作当前事件来反馈；hidden/partial 上帝视角只能后台校验，不能写进可见梗概。`;
        try {
            const res = await fetch('/api/chat/deduce', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
            ...
            sopEventBriefState.chat.push({ role: 'assistant', content: `已按你的意见更新事件 ${ctx.chapterNumber || ''} 的梗概。` });
...
【本地资料命中片段】\n${snippets}`;
    ...
    function getAdjacentEventContext(chapterNumber) {
        ...
        return {
            ...
            endInfo: next
                ? `后续边界：事件 ${next.chapter_number}《${next.title}》。只允许让当前事件结尾自然过渡到这个入口，禁止展开、改写、评价或推演该后续事件的具体情节。`
                : '暂无下一事件；请把当前事件自身的结果作为本段结束锚点，并提醒作者需要补充下一部分开始事件。'
        };
    }

    function findBibleChapterForContext(chapterNumber, title = currentLocalContext.title, bible = getCurrentBibleSnapshot()) {
        const chapters = Array.isArray(bible?.chapters) ? bible.chapters : [];
        const targetNumber = String(chapterNumber || '').trim();
        const targetTitle = normalizeStableKey(title || '');
        return chapters.find(ch => String(ch.chapter_number || '').trim() === targetNumber)
            || chapters.find(ch => targetTitle && normalizeStableKey(ch.title || '') === targetTitle)
            || null;
    }

    function getBibleCharactersForEvent(chapter = null, bible = getCurrentBibleSnapshot()) {
        if (!chapter || !Array.isArray(bible?.characters)) return [];
        const eventText = normalizeStableKey(`${chapter.title || ''}\n${chapter.content || ''}\n${chapter.description || ''}\n${JSON.stringify(chapter)}`);
        return bible.characters
            .filter(char => char?.name && eventText.includes(normalizeStableKey(char.name)))
            .map(char => applyManualBibleEditsToValue(char));
    }
...
7. 信息权限：未揭露或部分揭露的上帝视角秘密只能用于后台因果校验，不能让角色或观众提前知道；可见 SOP 提问、事件梗概、大纲和正文只能基于观众视角推进，揭露后才可公开调用上帝视角。
...
如果没有明显问题，必须明确写“未发现明显问题 / 当前可继续”，并且不要输出红色警报或黄色警报内容。`;
    ...
    function extractSopSupervisionSection(reply = '', heading = '红色警报') {
        const text = String(reply || '');
        const match = text.match(new RegExp(`【${heading}】([\\s\\S]*?)(?=\\n?【[^】]+】|$)`));
        return match ? match[1].trim() : '';
    }

    function hasBlockingSopSupervision(reply = '') {
        const redText = extractSopSupervisionSection(reply, '红色警报');
        if (!redText || /^(无|暂无|没有|未发现|当前可继续|无明显问题)/.test(redText)) return false;
        return /(无法|矛盾|泄露|降智|阻断|不通过|严重|违反|必须先|不能继续|冲突)/.test(redText);
    }

    function hasYellowSopSupervision(reply = '') {
        const yellowText = extractSopSupervisionSection(reply, '黄色提醒');
        if (!yellowText || /^(无|暂无|没有|未发现|当前可继续|无明显问题)/.test(yellowText)) return false;
        return /(风险|不足|建议|偏弱|缺口|需要|可优化|提醒)/.test(yellowText);
    }
...
请只输出：
【红色警报】会导致当前事件无法对齐前后事件、人物/时间线矛盾、信息权限泄露、人物降智的阻断问题；
【黄色提醒】动机偏弱、吸引力不足、伏笔/资料/规则不足的问题；
【当前最小修正】${mode === 'auto' ? '直接给出你建议采用的修正版本。' : mode === 'semi' ? '给 2-3 个可选修正方向。' : '只给建议，不改设定。'}
【下一步应该问作者】仅当没有红色警报时，给 1-2 个必须确认的问题；如果存在红色警报，本栏必须写“先消除红色警报，不提出新问题”。
如果没有问题，请明确“当前可继续”。如果没有任何冲突，不要伪造红色或黄色警报。`;
        ...
        try {
            ...
            if (hasBlockingSopSupervision(reply)) {
                renderSopSupervisionGate(reply);
                setSandboxAlert('red', `SOP 实时监督发现阻断风险。\n${limitText(reply, 700)}`);
            } else if (hasYellowSopSupervision(reply)) {
                setSopSupervisionBlocked(false);
                renderDeviationItems([reply]);
...
【本章 character_state_tracks】
${JSON.stringify(relevantTracks, null, 2)}

请生成“章节写作包”。章节包正文侧不能包含完整上帝视角，只能包含观众视角、角色视角、can_hint、must_hide。必须从 event_nodes 和人物状态轨道提取，不得新增重大剧情。
如果本章没有需要隐藏的上帝视角，information_permissions.must_hide 输出空数组即可；不要为了凑字段编造禁区。
...
只输出 JSON：
{
  ...
  "acceptance_checks": []
    ...
    async function generateChapterPackage() {
        ...
        const key = getCurrentChapterPipelineKey();
        chapterPipelineState.chapter_packages = { ...(chapterPipelineState.chapter_packages || {}), [key]: { ...normalizeChapterPackageForCurrentEvent(parsed), audit_status: '待检测' } };
        setPipelineStatus('章节包待检测', `已生成事件 ${currentLocalContext.chapterNumber}《${currentLocalContext.title}》章节包，请检测后再扩写。`);
    }

    function normalizeChapterPackageForCurrentEvent(pkg = {}) {
        const cut = getCurrentChapterCut();
        const currentNodes = getCurrentEventNodes();
        const nodeIds = cut?.node_ids?.length
            ? cut.node_ids.filter(id => currentNodes.some(node => node.node_id === id))
            : currentNodes.map(node => node.node_id).filter(Boolean);
        const mustWrite = Array.isArray(pkg.must_write) && pkg.must_write.length
            ? pkg.must_write
            : currentNodes.map(node => node.result || node.summary).filter(Boolean);
        const sceneChain = Array.isArray(pkg.scene_chain) && pkg.scene_chain.length
            ? pkg.scene_chain
            : currentNodes.map(node => ({
                node_id: node.node_id,
                scene_goal: node.summary || node.quality_function || '',
                conflict: node.obstacle || '',
                outcome: node.result || ''
            }));
        const permissions = pkg.information_permissions || {};
        return {
            ...pkg,
            chapter_identity: {
                ...(pkg.chapter_identity || {}),
                chapter_id: currentLocalContext.chapterId || getCurrentChapterPipelineKey(),
                chapter_number: Number(currentLocalContext.chapterNumber || pkg.chapter_identity?.chapter_number || 1),
                title: currentLocalContext.title || pkg.chapter_identity?.title || '',
                source_event_nodes: nodeIds
            },
            chapter_goal: pkg.chapter_goal || cut?.chapter_goal || currentNodes[0]?.summary || '完成当前事件推进。',
            opening_state: pkg.opening_state || cut?.opening_state || currentNodes[0]?.trigger || currentNodes[0]?.summary || '承接上一事件状态。',
            ending_state: pkg.ending_state || cut?.ending_state || currentNodes[currentNodes.length - 1]?.result || '抵达下一事件触发点。',
            must_write: mustWrite,
            may_write: Array.isArray(pkg.may_write) ? pkg.may_write : [],
            must_not_write: Array.isArray(pkg.must_not_write) ? pkg.must_not_write : [],
            active_characters: Array.isArray(pkg.active_characters) ? pkg.active_characters : [],
            information_permissions: {
                audience_knows: Array.isArray(permissions.audience_knows) ? permissions.audience_knows : [],
                pov_character_knows: Array.isArray(permissions.pov_character_knows) ? permissions.pov_character_knows : [],
                other_characters_know: Array.isArray(permissions.other_characters_know) ? permissions.other_characters_know : [],
                can_reveal: Array.isArray(permissions.can_reveal) ? permissions.can_reveal : [],
                can_hint: Array.isArray(permissions.can_hint) ? permissions.can_hint : [],
                must_hide: Array.isArray(permissions.must_hide) ? permissions.must_hide : []
            },
            rules_constraints: Array.isArray(pkg.rules_constraints) ? pkg.rules_constraints : [],
            foreshadowing: pkg.foreshadowing || { must_payoff: [], can_plant: [] },
            scene_chain: sceneChain,
            style_target: pkg.style_target || {},
            acceptance_checks: Array.isArray(pkg.acceptance_checks) ? pkg.acceptance_checks : []
        };
    }
...
【当前事件 character_state_tracks】
${JSON.stringify(normalizePipelineArray(chapterPipelineState.character_state_tracks).map(track => ({
    ...track,
    states: normalizePipelineArray(track.states).filter(state => (pkg.chapter_identity?.source_event_nodes || []).includes(state.node_id))
})).filter(track => track.states.length), null, 2)}

你是章节包闸门。请检查：章节包是否只包含本章信息；是否继承正确人物状态；是否隐藏上帝视角；must_not_write 是否覆盖关键禁区；是否足够支撑正文扩写；是否提前越过下一事件。
只有出现明确泄露上帝视角、违反世界规则、引用不存在节点、提前越过下一事件、缺少 must_write/scene_chain/opening_state/ending_state 时，才判定不通过。风格、精彩度、吸引力、节奏建议只列为黄色问题，不要阻断。
...
【最小修复建议】`;
        ...
        const failed = /【结论】\s*不通过|结论[:：]\s*不通过/.test(report || '')
            && /红色问题[\s\S]*(泄露|违反世界|违反规则|不存在节点|引用不存在|提前越过|缺少 must_write|缺少 scene_chain|缺少 opening_state|缺少 ending_state)/.test(report || '');
        const key = getCurrentChapterPipelineKey();
        chapterPipelineState.chapter_packages[key] = { ...normalizeChapterPackageForCurrentEvent(pkg), audit_status: failed ? '不通过' : '通过', audit_report: report };
        const softWarning = /【结论】\s*不通过|结论[:：]\s*不通过|红色问题|黄色问题/.test(report || '');
        setPipelineStatus(failed ? '章节包检测未通过' : '可扩写正文', failed ? report : `${softWarning ? '【黄色提示】章节包有建议，但不阻断正文扩写。\n\n' : ''}${report}`);
...
3. 禁止提前解决下一事件，禁止把下一事件的核心内容写进当前事件；下一事件只能作为“当前事件结尾停靠的边界”，不得展开其细节。
...
7. 所有隐藏/部分揭露的上帝视角只能作为后台校验；正文可见内容只能写观众视角已经能看到、误判、怀疑或接触到的线索。
    ...
    window.loadChapterContext = async function loadChapterContext(chapterId, chapterNumber, title) {
        ...
        try {
            ...
            if (data.success) {
                ...
                const bibleSnapshot = getCurrentBibleSnapshot();
                const bibleChapter = findBibleChapterForContext(chapterNumber, displayTitle, bibleSnapshot);
                const bibleCharacters = getBibleCharactersForEvent(bibleChapter, bibleSnapshot);
                const mergedCharacters = dedupeCharactersByIdentity([...(displayCharacters || []), ...(bibleCharacters || [])]);
                const effectiveSynopsis = bibleChapter?.content || displayChapter.content || "";
                const effectiveTitle = bibleChapter?.title ? applyManualCharacterRenamesToText(bibleChapter.title, manualEdits) : displayTitle;
                currentLocalContext = { chapterId, chapterNumber, title: effectiveTitle, synopsis: effectiveSynopsis, characters: mergedCharacters || [], hooks: displayHooks || [] };
                if (bibleChapter) {
                    workspaceChapters = workspaceChapters.map(ch => Number(ch.chapter_number) === Number(chapterNumber)
                        ? { ...ch, title: effectiveTitle, content: effectiveSynopsis }
                        : ch);
                }
                if (currentChapterTitle) currentChapterTitle.innerText = `事件 ${chapterNumber}：${effectiveTitle}`;
                if (editorTextarea) editorTextarea.value = displayChapter.content_text || "";
                if (editorSopConflict) editorSopConflict.innerText = effectiveSynopsis ? effectiveSynopsis : '尚未生成大纲，请在上方推演室讨论后提取。';

                // 💥 世界观强制重载，修复不显示的问题
                await loadProjectSettings();

                const eventContext = getAdjacentEventContext(chapterNumber);
                const charNames = mergedCharacters && mergedCharacters.length > 0 ? mergedCharacters.map(c => c.name).join('、') : '暂无指定人物';
                ...
                if (localDeviationPanel) {
                    ...
                    if (!mergedCharacters || mergedCharacters.length === 0) warnings.push('本章尚未绑定可调用角色，人物行为容易发散。');
                    if (bibleChapter && String(displayChapter.content || '').trim() !== String(bibleChapter.content || '').trim()) warnings.push('已用沙盒最新事件设定校准本章；数据库旧大纲与沙盒 Bible 不一致。');
                ...
                const aiGreeting = window.OmniPrompts?.chapterSopIntro
                    ? window.OmniPrompts.chapterSopIntro(chapterNumber, effectiveTitle, eventContext.endInfo, charNames)
...
11. 本事件结尾必须能自然停在后续边界前；不得展开下一事件正文内容、具体行动、反派计划、人物选择或设定细节。
 ...
 async function checkInitialConcept() {
    ...
    if (btnSendChapterChat) {
        btnSendChapterChat.onclick = async () => {
            ...
            if (!text) return;
            if (sopSupervisionGate.blocked) {
                openSopWarningFixModal(sopSupervisionGate.reason);
                return alert(`SOP 实时监督发现红色阻断风险，必须先在独立警告窗口处理或暂时忽略后，才能继续普通 SOP 对话。\n\n${limitText(sopSupervisionGate.reason, 700)}`);
            }
...
3. 每次提出事件细节，都要说明：行动人物、行为来源、冲突对象、不可逆后果、如何让当前事件结尾停在后续边界前；不得描述下一事件内部发生什么。
...
8. 用信息权限防火墙校验信息释放：未揭露/部分揭露的秘密只能用观众视角推进推理，上帝视角只能后台校验伏笔，不能提前泄露；已揭露后才可公开调用。不要把 hidden/partial 的 god_view 写成可见提问。
...
11. 下一事件只作为结尾衔接目标，不能把 SOP 讨论变成两个事件的联合推演；不得引用、评价、修改、补全下一事件的具体情节。
...
14. SOP 是有限流程，不允许无限询问。你必须按顺序完成并标记：当前事件理解 -> 人物选择/行为来源 -> 阻力/代价/对抗 -> 信息释放/观众视角/揭露权限 -> 伏笔种植/回收 -> 章数方案或分章确认。上述全部完成后，必须输出：
【SOP_READY】
...
16. 提问必须编号为 Q1、Q2、Q3...。如果作者一口气回答了多个问题，先在【已吸收】列出“Qx 已回答/部分回答/冲突：摘要”，后续不要重复问已回答问题。作者额外提出的重要设定要写入【新增重要设定】，编号为 S1、S2...，并建议写入人物卡/人物规则/事件/规则/上帝视角/伏笔/暂存。`;
                ...
                if (lastUserMsg && lastUserMsg.role === 'user') {
                    ...
                    lastUserMsg.content += `\n\n${hiddenWorkflow}` + (currentLocalContext.hookAlert || "") + buildInteractionFocusPrompt('sop', text) + `\n\n[统一监督指令]：请严格遵循沙盒最终设定继承契约、规则/专家资料、救猫咪类型、人物档案、信息权限防火墙和长篇编辑状态推演，严禁专业乱写、逻辑跳步、人物降智、OOC或提前泄露真相。\n\n${sopCanonContract}\n\n【救猫咪类型监督】：\n${getSaveTheCatGenreGuide(getCurrentStoryGenre())}\n【统一规则/专家资料】：\n${worldRules}\n【人物卡】：\n${characterDetails}\n【上帝视角信息权限】：\n${godViewContext}\n\n${buildInformationFirewallPrompt()}\n【长篇编辑状态】：\n${getLongformEditorialContext()}`;
                }
