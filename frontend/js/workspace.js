...
6. 不要写正文。`;
    ...
    async function generateSopEventBrief(context = getSopEventBriefModalContext()) {
        const eventContext = getAdjacentEventContext(context.chapterNumber);
...
1. 必须以【本事件故事梗概】开头。
2. 只讲当前事件，不展开下一事件。
3. 按事件发生顺序写清：起因、关键行动、阻力升级、人物选择、代价、信息释放、伏笔种植/回收、结尾如何停在后续边界前。
...
5. 不写正文，不写分章大纲。`;
    ...
    async function sendSopEventBriefRevision() {
        ...
        const eventContext = getAdjacentEventContext(ctx.chapterNumber);
        const history = getSopHistoryForChapter(ctx.chapterId);
...
请输出更新后的完整【事件 ${ctx.chapterNumber || ''} 故事梗概】，并保留【顺序与细节检查】。不要把下一事件当作当前事件来反馈。`;
        try {
            const res = await fetch('/api/chat/deduce', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
